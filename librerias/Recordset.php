...
class Recordset { var $manejador = "mysql";var $servidor = "localhost";var $usuario = "root";var $clave = "1234";var $puerto = "";var $sql = "";var $limite_inicio = 0;var $limite_cantidad = 0;var $bd = "sicca";var $conexion;var $tabla;var $fila;var $posicion = -1;var $total_registros;var $total_campos;var $total_paginas;var $pagina_anterior;var $pagina_siguiente; function getBD(){ return $this->bd; }
	...
	function abrir()
	{
		switch ($this->manejador)
		{
			case "mysql":
				...
				$this->conexion = @mysql_pconnect($this->servidor, $this->usuario, $this->clave) or die("<strong>".htmlentities("¡Servidor, usuario o clave incorrecta!")."</strong>");
			...
			case "mssql":
				$this->conexion = @mssql_pconnect ($this->servidor, $this->usuario, $this->clave) or die("<strong>".htmlentities("¡Servidor, usuario o clave incorrecta!")."</strong>");
	...
	function desplazar()
	{
		...
		{
			echo "<strong>¡la posición esta fuera del rango!</strong>";
		}
	}
	...
	function crearpaginador($ruta,$target,$rutaimagen,$condicion = "")
	{
		if($this->total_paginas > 1)
		{
			...
			echo "\n\t<tr>";
			echo "\n\t\t<td>";
			$this->paginarinicio($ruta,$target,$rutaimagen."inicio.png",$condicion);
			echo "</td>";
			echo "\n\t\t<td>";
			$this->paginaratras($ruta,$target,$rutaimagen."atras.png",$condicion);
			echo "</td>";
			echo "\n\t\t<td>P&aacute;gina ".$this->pagina_actual." / ".$this->total_paginas."</td>";
			echo "\n\t\t<td>";
			$this->paginarsiguiente($ruta,$target,$rutaimagen."siguiente.png",$condicion);
			echo "</td>";
			echo "\n\t\t<td>";
			$this->paginarfin($ruta,$target,$rutaimagen."ultimo.png",$condicion);
			echo "</td>";
			echo "\n\t</tr>";
			echo "\n</table>";
		}
	...
	<tr>
		<td><img style="cursor:pointer" title="Primera P&aacute;gina" src="'.$rutaimagen.'inicio.png" onclick="'.$ajax.'(\''.$archivo.'?pagina=1'.$condicion.'\');" /></td>
		<td><img style="cursor:pointer" title="Anterior P&aacute;gina" src="'.$rutaimagen.'atras.png" onclick="'.$ajax.'(\''.$archivo.'?pagina='.$this->pagina_anterior.$condicion.'\');" /></td>
		<td><b>'.$this->pagina_actual." / ".$this->total_paginas.'</b></td>
		<td><img style="cursor:pointer" title="Siguiente P&aacute;gina" src="'.$rutaimagen.'siguiente.png" onclick="'.$ajax.'(\''.$archivo.'?pagina='.$this->pagina_siguiente.$condicion.'\');" /></td>
		<td><img style="cursor:pointer" title="&Uacute;ltima P&aacute;gina" src="'.$rutaimagen.'ultimo.png" onclick="'.$ajax.'(\''.$archivo.'?pagina='.$this->total_paginas.$condicion.'\');" /></td>
	</tr>
</table>';
		if($this->total_paginas == 1)
			$html = "&nbsp;";
	...
	function llenarcombo($opciones = "", $checked = "", $fukcion = "", $diam = "", $disab="")
	{
		echo "<select name=$opciones id=$opciones $fukcion $diam $disab>";
