...
var r = "";
	if(res!=false){
		r = "modulos/correspondencia/procesar/bsq_correspondencia.php?pagina=1&condicion="+res;
...
//9
function la(q){
	window.frames.corres.location.href='modulos/correspondencia/procesar/procesar.php?id_recepcion='+q;
	closeMessage();	
}
// 10
...
//11
function invo(h){
	if(h=="si"){ document.getElementById("_varios_n_oficio_ext").value=""; document.getElementById("n_oficio_ext").value = ""; document.getElementById("uno").style.display=""; document.getElementById("div_oficio").style.display="none"; document.getElementById("div_oficio_mas").style.display="";
	} else {
				document.getElementById("uno").style.display="none"; 
				document.getElementById("div_oficio").style.display="";
				document.getElementById("div_oficio_mas").style.display="none";
				
				var My_path = "modulos/correspondencia/generar/lis_organismos.php?acc=cancelar";
...
/*function age(y){
	if(document.getElementById("si").checked == true){	
		...
		<? 
/*			$ma = new Recordset();
			$ma->sql = "SELECT id_campo2 FROM temp";			
			$ma->abrir();
...
/*				for($i=0;$i<$ma->total_registros;$i++)
				{
					$ma->siguiente();
*/		?>
					contenedor[<? //echo $i; ?>]= "<? //echo $ma->fila["id_campo2"]; ?>";
		<?
/*				}
				$elultimo = $ma->fila["id_campo2"];
				$transi=(int)$elultimo;
				$transi++;		
...
*/		?>
		...
		var i = ""; 
			for (i=0;i<contenedor.length;i++){
				if(document.getElementById("_varios_n_oficio_ext").value==contenedor[i]){
					alert(acentos("El n&deg; de oficio que introdujo ya ha sido utilizado, por favor verifique!!"));
					return;
				}
			}  
			
			if(nuevo != document.getElementById("_varios_n_oficio_ext").value){
		...
		if(document.getElementById("id_organismo_reci").value == null || document.getElementById("id_organismo_reci").value.length == 0 || /^\s+$/.test(document.getElementById("id_organismo_reci").value)){
			...
			return false;
		} else if (document.getElementById("_varios_n_oficio_ext").value == null || document.getElementById("_varios_n_oficio_ext").value.length == 0 || /^\s+$/.test(document.getElementById("_varios_n_oficio_ext").value)){
			alert(acentos("El campo N&deg; Oficio/Circular est&aacute; vacio, por favor completelo"));
			document.getElementById("_varios_n_oficio_ext").select();	
			return false;
		} else {
			var My_path = "modulos/correspondencia/generar/lis_organismos.php?acc=agregar&id_or="+y+"&ofi="+document.getElementById("_varios_n_oficio_ext").value;
			var spasc = mostrarContenido3;
			llamarlistado(My_path,spasc);	
			document.getElementById("organismo_reci").value = "";
			document.getElementById("id_organismo_reci").value = "";
			document.getElementById("_varios_n_oficio_ext").value = "";		
		}
	}
}*/
// 13
...
/*	if (document.getElementById("mensaje").value == null || document.getElementById("mensaje").value.length == 0 || /^\s+$/.test(document.getElementById("mensaje").value)){
	...
	{
		if (document.getElementById("destina").value == null || document.getElementById("destina").value.length == 0 || /^\s+$/.test(document.getElementById("destina").value)){
				alert(acentos("El campo Destinatario est&aacute; vacio, por favor completelo"));
				document.getElementById("destina").focus();
				return false;
			}
	}
	...
	if(con==0){
		alert(acentos("El N&deg; Oficio/Circular ya ha sido utilizado!!, por favor use otro.")); 
	...
	if (document.getElementById("f_registro").value == null || document.getElementById("f_registro").value.length == 0 || /^\s+$/.test(document.getElementById("f_registro").value)){
			alert(acentos("El campo Fecha Registro est&aacute; vacio, por favor completelo"));
			document.getElementById("f_registro").focus();
			return false;
		}		
...
//17
function eli(v){
	var My_path = "modulos/correspondencia/generar/lis_organismos.php?acc=eliminar&id_or="+v;
	var spasc = mostrarContenido3;
	llamarlistado(My_path,spasc);	
}
//18
function limp(){
	...
	document.getElementById("org_cgr").checked = true;
...
function sali(q){
	if(q=="org_cgr"){
		document.getElementById("salida_1").style.display="";
		document.getElementById("salida_2").style.display="none";					
		document.getElementById("destina").value = "";
	} else if (q=="otro") {
		document.getElementById("salida_1").style.display="none";
		document.getElementById("salida_2").style.display="";					
		document.getElementById("organismo_reci").value = "";
		document.getElementById("id_organismo_reci").value = "";			
	}
}
