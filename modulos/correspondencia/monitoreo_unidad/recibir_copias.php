<?
if(!stristr($_SERVER['SCRIPT_NAME'],"recibir_copias.php")){
...
$requi_oficio = stripslashes($_GET["oficioext"]);
if ($requi_oficio=="si"){

if ((isset($recepcion) && $recepcion !="")){	
	if(ctype_digit($recepcion)){	
		$search = new Recordset();
		$search->sql = "SELECT id_asignacion_correspondencia FROM crp_asignacion_correspondencia WHERE (id_recepcion_correspondencia = '".$recepcion."') AND (copia_unidades LIKE '%".$UunidadD."%')";
			$search->abrir();
		...
		<td align="center">
			<table width="99%" align="center" cellpadding="2" cellspacing="0">
				<tr>
					...
					<td class="titulomenu" valign="middle">Certificar Recepci&oacute;n Copia de Correspondencia</td>
				</tr>
				<tr>
...
<?
							...
							<tr>
								<td align="center">
									...
									<input type="button" name="btnRegistrar" id="btnRegistrar" value="Recibir" title="Recibir" onclick="recibirCopia(this.value);" />
									&nbsp;&nbsp;
...
<? } else if ($requi_oficio=="no"){ 

if ((isset($recepcion) && $recepcion !="")){	
	if(ctype_digit($recepcion)){	
		$search = new Recordset();
		$search->sql = "SELECT id_asignacion_correspondencia FROM crp_asignacion_correspondencia WHERE (id_recepcion_correspondencia = '".$recepcion."') AND (externo = 0)";
			$search->abrir();
			if($search->total_registros != 0)
			{
				$bus = new Recordset();
	...
	<tr>
		<td align="center">
			<table width="99%" align="center" cellpadding="2" cellspacing="0">
				<tr>
					<td width="30px"><img src="images/asignadas.png"/></td>
					<td class="titulomenu" valign="middle">Certificar Recepci&oacute;n de Correspondencia</td>
				</tr>
				<tr>
					<td colspan="2" valign="top"><hr class="barra" /></td>
				</tr>
