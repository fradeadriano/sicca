...
$sql = "select organismo, id_organismo from organismo where organismo LIKE '%$q%' AND cgr = 1";
$rsd = mysql_query($sql);
while($rs = mysql_fetch_array($rsd)) {
	$cid = $rs['id_organismo'];
	$cname = html_entity_decode($rs['organismo'], ENT_COMPAT, 'UTF-8');
	echo "$cname|$cid\n";
}echo "<span style='font-weight:bold; color:#990000;'>".html_entity_decode("No ex&iacute;ste el nombre tipeado", ENT_COMPAT, 'UTF-8')."</span>"; 
?>
