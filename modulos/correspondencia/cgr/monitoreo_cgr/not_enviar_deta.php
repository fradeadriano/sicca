<?
if(!stristr($_SERVER['SCRIPT_NAME'],"not_enviar_deta.php")){
			...
			<table border="0" align="center" class="b_table" width="100%" height="400" bgcolor="#FFFFFF">			
				<tr valign="top">
					<td align="center">
						...
						<table border="0" cellpadding="3" ccellspacing="2" width="99%">
							<tr><td></td><td height="5px"><span style="font-size:9px">N&deg; Notificaci&oacute;n / Ciudadano-Organ&iacute;smo / Tel&eacute;fono</span></td></tr>
<?
								$decisi = stripslashes($_GET["ac"]);
								if ($decisi=="env"){									
?>								
							<tr>
								...
								<td valign="bottom">
									<? 
									$rsun = new Recordset();
									$rsun->sql = "SELECT crp_recepcion_cgr_detalle.`id_recepcion_cgr_detalle`, CONCAT(crp_recepcion_cgr_detalle.`n_notificacion`,' / ',IF(crp_recepcion_cgr_detalle.`id_organismo` IS NULL, crp_recepcion_cgr_detalle.`nombre`,organismo.`organismo`),IF(crp_recepcion_cgr_detalle.`telefono` IS NOT NULL,CONCAT(' / ',crp_recepcion_cgr_detalle.telefono),'')) AS autor
													FROM crp_recepcion_cgr_detalle LEFT JOIN organismo ON (crp_recepcion_cgr_detalle.`id_organismo` = organismo.`id_organismo`)
													WHERE crp_recepcion_cgr_detalle.`enviado` = 1 AND crp_recepcion_cgr_detalle.`id_recepcion_correspondencia_cgr` = ".$recepcion; 
									$rsun->llenarcombo($opciones = "\"notificado\"", $checked = "", $fukcion = "" , $diam = "style=\"width:580px; Height:20px;\""); 
									$rsun->cerrar(); 
									unset($rsun);																						
							...
							<tr>
								<td align="right" valign="top" width="132">
									<b>Fecha Envio:</b>&nbsp;
								</td>
								<td>
...
?>
							<tr>
								...
								<td valign="bottom">
									...
									$rsun->sql = "SELECT crp_recepcion_cgr_detalle.`id_recepcion_cgr_detalle`, CONCAT(crp_recepcion_cgr_detalle.`n_notificacion`,' / ',IF(crp_recepcion_cgr_detalle.`id_organismo` IS NULL, crp_recepcion_cgr_detalle.`nombre`,organismo.`organismo`),IF(crp_recepcion_cgr_detalle.`telefono` IS NOT NULL,CONCAT(' / ',crp_recepcion_cgr_detalle.telefono),'')) AS autor
													FROM crp_recepcion_cgr_detalle LEFT JOIN organismo ON (crp_recepcion_cgr_detalle.`id_organismo` = organismo.`id_organismo`)
													WHERE crp_recepcion_cgr_detalle.`entregado` = 1 AND crp_recepcion_cgr_detalle.`finalizado` = 'no' AND crp_recepcion_cgr_detalle.`id_recepcion_correspondencia_cgr` = ".$recepcion; 
									$rsun->llenarcombo($opciones = "\"notificado\"", $checked = "", $fukcion = "" , $diam = "style=\"width:580px; Height:20px;\""); 
									$rsun->cerrar(); 
									unset($rsun);																						
									?>&nbsp;<span class="mensaje">*</span>
							...
							<tr>
								<td valign="bottom"  align="right" width="132">
									<b>Se localiz&oacute;:</b>&nbsp;
								</td>
								<td>
									Si&nbsp;<input type="radio" name="localizado" id="si"  checked="checked" onclick="habi_fecha_entre(this.id);"/>&nbsp;&nbsp;No&nbsp;<input type="radio" name="localizado" id="no" onclick="habi_fecha_entre(this.id);" />
								</td>
							</tr>
							<tr id="fecha_entrega_notificacion">
								<td colspan="2">		
...
?>
						...
						{																
							$mostr = new Recordset();
							$mostr->sql = "SELECT crp_recepcion_cgr_detalle.`id_recepcion_cgr_detalle`,crp_recepcion_cgr_detalle.`n_notificacion` AS notificacion,IF(crp_recepcion_cgr_detalle.`id_organismo` IS NULL,crp_recepcion_cgr_detalle.`nombre`,organismo.`organismo`) AS autor, 
											IF(crp_recepcion_cgr_detalle.`telefono` IS NOT NULL,crp_recepcion_cgr_detalle.telefono,'-') AS telefono, DATE_FORMAT(crp_recepcion_cgr_detalle.fecha_envio,'%d/%m/%Y') AS envio, IF(crp_recepcion_cgr_detalle.fecha_entrega IS NULL, '-', DATE_FORMAT(crp_recepcion_cgr_detalle.fecha_entrega,'%d/%m/%Y')) AS entrega,  
							...
							<tr>
								<td colspan="2">
									<table border="0" cellpadding="3" ccellspacing="2" width="99%" class="b_table1">								
										<tr class="trcabecera_list1"><td colspan="5">Notificaciones Enviadas</td></tr>
							...
							<tr>
								<td align="center">								
									<?
										$decisi = stripslashes($_GET["ac"]);
										if ($decisi=="env"){
											echo '<input type="button" name="btnRegistrar" id="btnRegistrar_varios" value="Registrar" title="Registrar Envio" onclick="reg_envio_varios(this.id);" />';
										} elseif ($decisi=="ent"){
											echo '<input type="button" name="btnRegistrar" id="btnRegistrar" value="Registrar" title="Registrar Entrega" onclick="reg_envio_varios(\'Finalizar_varios\');" />';
										}
									?>
