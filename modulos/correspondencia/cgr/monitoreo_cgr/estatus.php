<?
	...
	$search1->sql = "SELECT COUNT(crp_asignacion_correspondencia_cgr.id_estatus) AS total, estatus.estatus 
					...
					WHERE date_format(crp_asignacion_correspondencia_cgr.fecha_asignacion,'%Y') = 2015 AND crp_asignacion_correspondencia_cgr.`id_estatus` BETWEEN 1 AND 7
					GROUP BY crp_asignacion_correspondencia_cgr.id_estatus ORDER BY total DESC";			
	$search1->abrir();
