...
$q = strtolower($_GET["q"]);
if (!$q) return;
	if (stripslashes($_GET["op"])=="ccgr") {
		$sql = "select organismo, id_organismo from organismo where cgr <> 0 AND organismo LIKE '%$q%'";
	} else if (stripslashes($_GET["op"])=="ext") {
...
while($rs = mysql_fetch_array($rsd)) {
	...
	echo "$cname|$cid\n";
}echo "<span style='font-weight:bold; color:#990000;'>".html_entity_decode("Si no Ex&iacute;ste.! Agreguelo desde el menu Mantenimiento seguido la opci&oacute;n Organ&iacute;smos", ENT_COMPAT, 'UTF-8')."</span>"; 
?>
