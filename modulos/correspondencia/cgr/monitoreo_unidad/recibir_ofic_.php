...
if ((isset($recepcion) && $recepcion !="")){	
	if(ctype_digit($recepcion)){	
		...
		echo $search->sql = "SELECT id_asignacion_correspondencia FROM crp_asignacion_correspondencia WHERE (id_recepcion_correspondencia = '".$recepcion."') AND (copia_unidades LIKE '%".$UunidadD."%')";
			$search->abrir();
			if($search->total_registros != 0)
			{
				$bus = new Recordset();
				echo $bus->sql = "SELECT crp_recepcion_correspondencia.n_correlativo, DATE_FORMAT(crp_recepcion_correspondencia.fecha_documento,'%d/%m/%Y') AS fdocumento, DATE_FORMAT(crp_recepcion_correspondencia.fecha_registro,'%d/%m/%Y %r') AS registro, crp_recepcion_correspondencia.n_documento,
