...
$operacion = stripslashes($_GET["opera"]);

if ((isset($recepcion) && $recepcion !="")){	
	if(ctype_digit($recepcion)){	
		$search = new Recordset();
		$search->sql = "SELECT id_crp_asignacion_correspondencia_cgr FROM crp_asignacion_correspondencia_cgr WHERE (id_recepcion_correspondencia_cgr = '".$recepcion."') AND (id_unidad = '".$UunidadD."')";
			$search->abrir();
			if($search->total_registros != 0)
			{
				$bus = new Recordset();
				$bus->sql = "SELECT DATEDIFF(crp_recepcion_correspondencia_cgr.fecha_vencimiento, CURRENT_DATE()) AS tiempo,
	...
	<tr>
		<td align="center">
			<table width="99%" align="center" cellpadding="2" cellspacing="0">
				<tr>
					<td width="30px"><img src="images/<? if($operacion=="Nohabil"){echo "habilitado.png"; } else if($operacion=="habil") { echo "recibido.png";} ?>"/></td>
					<td class="titulomenu" valign="middle">Visualizar Correspondencia CGR</td>
				</tr>
				<tr>
				...
				<tr>
					<td align="center">								
						<? if ($operacion=="habil"){ ?>
							...
							<input type="button" name="btnRegistrar" id="btnRegistrar" value="Habilitar" title="Habilitar" onclick="habilitar();" />
							&nbsp;&nbsp;									
						<? };?>
