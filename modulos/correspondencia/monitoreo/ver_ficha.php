<?
if(!stristr($_SERVER['SCRIPT_NAME'],"ver_ficha.php")){
...
if ((isset($recepcion) && $recepcion !="")){	
	if(ctype_digit($recepcion)){	
		...
		$search->sql = "SELECT id_asignacion_correspondencia  FROM crp_asignacion_correspondencia WHERE (id_recepcion_correspondencia = '".$recepcion."')";
			...
			{
				$bus = new Recordset();
				$bus->sql = "SELECT IF(unidad.unidad IS NULL,'--', unidad.unidad) AS unidad, crp_recepcion_correspondencia.n_correlativo, DATE_FORMAT(crp_recepcion_correspondencia.fecha_documento,'%d/%m/%Y') AS fdocumento, DATE_FORMAT(crp_recepcion_correspondencia.fecha_registro,'%d/%m/%Y %r') AS registro, crp_recepcion_correspondencia.n_documento,
							  ...
							  crp_recepcion_correspondencia.gaceta_tipo, crp_recepcion_correspondencia.observacion, crp_recepcion_correspondencia.anexo,
							   ...
							   crp_asignacion_correspondencia.copia_unidades, crp_asignacion_correspondencia.unidades_apoyo, estatus.estatus, crp_asignacion_correspondencia.observacion as observ_asig
							FROM
				...
				if($bus->total_registros != 0)
				{
					...
					$observ_asig = $bus->fila["observ_asig"];											
	...
	<tr>
		<td align="center">
			<table width="99%" align="center" cellpadding="2" cellspacing="0">
				<tr>
					<td width="30px"><img src="images/recibido.png"/></td>
					<td class="titulomenu" valign="middle">Visualizar Correspondencia Asignada</td>
				</tr>
				<tr>
