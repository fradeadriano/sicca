...
function filtrar(co){
	...
	if(res==""){
		rt = "modulos/correspondencia/monitoreo_revisar/monitoreo_list.php?pagina=1&p_orden="+co+"&met="+metod; 
	} else {
		rt = "modulos/correspondencia/monitoreo_revisar/monitoreo_list.php?pagina=1&p_orden="+co+"&met="+metod+"&condiciones="+res; 	
	}	
	llamarlistado(rt,spasc);
}
//7
...
function doit(){
	...
	var ruta = "modulos/correspondencia/monitoreo_revisar/monitoreo_list.php?pagina=1&condiciones="+param+"&met=DESC";
//	alert(ruta);	
...
function recibir(){
	...
	if(window.confirm(acentos('Desea registrar la recepci&oacute;n de la correspondencia?'))) 
		{	
			rt = "modulos/correspondencia/monitoreo_revisar/reg_asignacion.php?recep="+document.getElementById("id_recep").value+"&modo=recibir"; 	
			llamarlistado(rt,spasc);
			alert(acentos("Ha finalizado satisfactoriamente la Recepci&oacute;n de la correspondencia!"));
			cargar_lista_corres('modulos/correspondencia/monitoreo_revisar/monitoreo_list.php?pagina=1&met=DESC');
			closeMessage();		
		}
}
...
function devolucion(){
	if(document.getElementById("motivo").value == null || document.getElementById("motivo").value.length == 0 || /^\s+$/.test(document.getElementById("motivo").value))
		{
			alert(acentos("El campo motivo esta vac&iacute;o!!"));
			return false;	
		}	
	...
	if(window.confirm(acentos('Desea registrar la Devoluci&oacute;n de la correspondencia?'))) 
		{	
			rt = "modulos/correspondencia/monitoreo_revisar/reg_asignacion.php?recep="+document.getElementById("id_recep").value+"&mot="+document.getElementById("motivo").value+"&modo=devolucion"; 	
			var spasc = mostrarContenido3;
			llamarlistado(rt,spasc);
			alert(acentos("Ha finalizado satisfactoriamente la Devoluci&oacute;n de la correspondencia!"));
			cargar_lista_corres('modulos/correspondencia/monitoreo_revisar/monitoreo_list.php?pagina=1&met=DESC');
			closeMessage();		
		}	
}
...
function mostrarContenido3(){
	document.getElementById("divi_devo").innerHTML = ajax.response;	
}
