<?
if(!stristr($_SERVER['SCRIPT_NAME'],"entregado.php")){
		...
		$search->sql = "SELECT id_asignacion_correspondencia  FROM crp_asignacion_correspondencia WHERE (id_recepcion_correspondencia = '".$recepcion."')";
			...
			if($search->total_registros != 0)
			{
				...
				$bus1->sql = "SELECT crp_correspondencia_externa.n_oficio_externo, 
										...
										mensajero.nombre_apellido, crp_correspondencia_externa.id_recepcion_correspondencia, crp_correspondencia_externa.id_correspondencia_externa
									FROM crp_correspondencia_externa INNER JOIN mensajero ON (crp_correspondencia_externa.id_mensajero = mensajero.id_mensajero)
				...
				if($bus1->total_registros != 0)
				{
					...
					$nombre_apellido = $bus1->fila["nombre_apellido"];	
